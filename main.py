...
@bot.event
async def on_ready():
    print(f'[{time.asctime(time.localtime(time.time()))}] BOT {bot.user} is ready to assist you!')
...
async def ping(inter, arg: str = "both"):
    ...
    match arg:
        case 'both':
          t = time.time()
          embedExample = disnake.Embed(title="Pong! Ожидание между клиентом и сервером...")
          tempMessage = await inter.channel.send(embed = embedExample)
          ping = round(((time.time() - t) * 1000), 0)
          await tempMessage.delete()
          embedExample = disnake.Embed(title=':ping_pong: PONG!', color=0x7FFF00)
          ...
          embedExample.add_field(name="**Ответ сервера:**", value=f"```{int(bot.latency * 1000)} ms```")
          embedExample.add_field(name="**Проверка сообщением:**", value=f"```{int(ping)} ms```")
        ...
        case 'latency':
            ...
            text="Проблемы со стороны Discord могут вызвать странную или высокую задержку",
            icon_url=SERVERICON_URL)
            embedExample.add_field(name="**Ответ сервера:**", value=f"```{int(bot.latency * 1000)} ms```", inline=True)
...
async def gf_RemainingTime(inter, short: bool = True):
    ...
    if not short:
      ...
      embed.add_field(name="**Наступит через:**", value="<t:1735696080:R>", inline=True)
      embed.add_field(name="**Отсчёт до:**", value="<t:1735696080:F>", inline=True)
...
async def gf_embed(inter, title : str = 'None', desc : str = 'None', color : int = 0x000000, authorname : str = 'None', authoricon : str = 'None', footertext : str = 'None', footericon : str = 'None'):
  ...
  date = datetime.now()
  month = date.month
  embed = disnake.Embed(title=(None if title == "None" else title), description=(None if desc == "None" else desc), color=int(color))
  ...
  if authorname != "None" or authoricon != "None":
    an, ai = False, False
    if authorname: an = True
    if authoricon:
      if authoricon.startswith('https://') or authoricon.startswith('http://'):
        if requests.get(authoricon).status_code == 200: ai = True

    embed.set_author(name=(authorname if an else None), icon_url=(authoricon if ai else None))
  ...
  if footertext != "None" or footericon != "None":
    ft, fi = False, False
    if footertext: ft = True
    if footericon:
      if footericon.startswith('https://') or footericon.startswith('http://'):
        if requests.get(footericon).status_code == 200: fi = True

    embed.set_footer(text=(footertext if ft else None), icon_url=(footericon if fi else None))
